...
A browser extension (Chrome + Firefox) that removes the  cookie/tracking-consent pop-up on ten Nordic news sites, restores the page scrolling that pop-up locks, and blocks the network requests that load it in the first place.
...
Both browser targets are built from the same source — one content script, one DNR ruleset — with only the manifest differing per browser (Firefox needs `browser_specific_settings`, Chrome doesn't).

## Requirements

- [Node.js](https://nodejs.org/) 20 or later (developed against 20.19.2)
- npm 10+ (bundled with Node; developed against 11.4.1)
- Chrome and/or Firefox, for loading/testing the built extension
...
This compiles the content script once and fans it out into two ready-to-load extensions: `dist/chrome` and `dist/firefox`. See [How to test](#how-to-test) below to load either one in a browser.

## Available scripts

| Script | What it does |
| --- | --- |
| `npm run dev` | Builds in watch mode — rebuilds `dist/chrome` and `dist/firefox` on every source change. |
...
| `npm run start:chrome` | Launches Chrome via `web-ext run --target chromium` with the extension pre-loaded from `dist/chrome`. |
| `npm run package:firefox` | Zips `dist/firefox` into `web-ext-artifacts/firefox/` for AMO submission. |
| `npm run package:chrome` | Zips `dist/chrome` into `web-ext-artifacts/chrome/` for Chrome Web Store submission. |
| `npm run package` | Runs both `package:firefox` and `package:chrome`. |
| `npm run lint` | Runs `web-ext lint` against `dist/firefox` (AMO-specific validation; no Chrome equivalent is bundled). |

## How to test

Run `npm run build` first so `dist/chrome` and `dist/firefox` exist.
...
Alternatively, `npm run start:chrome` launches a fresh Chrome instance with it already loaded — convenient for one-off testing, but changes still require re-running the script.
